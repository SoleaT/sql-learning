...
FROM users, profiles
WHERE user_id = id AND (TIMESTAMPDIFF(YEAR, birthday, NOW()) < 21);
...
SELECT firstname, lastname, COUNT(from_user_id) AS 'кол-во отправленных сообщений',
DENSE_RANK() 
	OVER (order BY COUNT(from_user_id) DESC) AS 'место в рейтинге'
FROM messages, users 
...
SELECT created_at, LAG(created_at, 0,0) 
	OVER (ORDER BY created_at) AS 'датa отправления',
TIMESTAMPDIFF(SECOND, LAG(created_at) 
	OVER (ORDER BY created_at), created_at) AS 'разница между соседними сообщениями/cек'
FROM messages;
